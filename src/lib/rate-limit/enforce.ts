...
function pickTier(request: NextRequest): RateLimitTier {
  ...
  // Search endpoint
  if (pathname === '/api/thoughts' && upperMethod === 'GET' && searchParams.has('searchTerm')) {
    return 'get:search';
  }
  ...
  if (
    ...
    (upperMethod === 'PUT' || upperMethod === 'DELETE' || upperMethod === 'PATCH')
  ) {
...
function rateLimitExceededResponse(res: RateLimiterRes, limitPoints: number): NextResponse<IError> {
  return jsonError(
    [
      {
...
function rateLimitFailClosedResponse(limitPoints: number): NextResponse<IError> {
  return jsonError(
    [
      {
        code: 'ratelimit/exceeded',
...
export async function enforceRateLimit(
  ...
  try {
    ...
    response.headers.set('X-RateLimit-Remaining', res.remainingPoints.toString());
    response.headers.set('X-RateLimit-Reset', resetSecondsFromNow(res.msBeforeNext).toString());
    return response;
  } catch (err) {
