...
import { flaggedLettersCountOptions, flaggedLettersPageOptions } from './options';
import SubmittedLetterPreviewModal from './submitted-letter-preview-modal';
import type { SubmissionLetter } from '@/types/submission';

export interface FlaggedLettersTabProps {
  isActive: boolean;
}

export default function FlaggedLettersTab({ isActive }: FlaggedLettersTabProps) {
  ...
  const { data: letters, isFetching } = useQuery({
    ...flaggedLettersPageOptions(page),
    enabled: isActive,
  });

  const { data: totalCount } = useQuery({
    ...flaggedLettersCountOptions(),
    enabled: isActive,
  });
